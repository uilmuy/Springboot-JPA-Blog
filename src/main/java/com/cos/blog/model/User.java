...
public class User {
	...
	@Column(nullable = false,length = 30,unique = true)//nullable=false는 null 값을 허용하지 않는다.
	...
	@Enumerated(EnumType.STRING) //해당 enum타입을 스트링이라고 알려준다.
	private RoleType role;  //Enum을 쓰는게 좋다. (admin,user,manager등 권한을 줄 수 있다.) roletype 강제
